...
module.exports = (server) => {
   ...
   *
   * This endpoint will be used to retrieve all reports saved by a user via the supplied user email
   *
   */
  server.get('/reports/findByEmail/:email', async(req, res, next) => {
    // get email from params
    try {
      const { email } = req.params;

      try {
        const result = await dataService.read(email, 'user_email', tableName);
