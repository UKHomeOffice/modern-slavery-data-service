...
const AuthenticationUtils = require('./util');
...
function isAuthorised(req, res, next) {
  const {isAuthorisationKeyValid} = AuthenticationUtils.checkAuthorisationKey;

  if (config.apiKeys.some(isAuthorisationKeyValid)) {
    next();
  } else {
...
function authenticateAPIKey(req, res, next) {
  DebugDB(`Authenticate: ${util.inspect(req.authorization)}`);
